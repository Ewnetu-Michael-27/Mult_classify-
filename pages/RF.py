...
no_n_est=st.slider("Choose n_estimators.", 1,30,3)
st.write("The choice of n_estimators is ", no_n_est)
st.text("")
st.write("Choose below a criterion to measure quality of split. **gini** is recommended!")
crit=st.selectbox("Criterion Choice", 
                             ("gini", "entropy", "ftrl", "log_loss"))
...
if st.session_state["button_pre_RF"]:
    ...
    if button_pre:
        ...
        #time for prediction
        p_a=model.predict(cont_predict_sc.reshape((1,len(cont_predict_sc))))[0]
